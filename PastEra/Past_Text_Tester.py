...
tree_pos = [(76, 112), (576, 400), (776, 162), (476, 162), (276, 302),
            (676, 12), (876, 312), (136, 412), (270, 70)]
enemy_coords = [(5*TILESIZE, 1*TILESIZE), (14*TILESIZE, 2*TILESIZE),
                (21*TILESIZE, 7*TILESIZE), (14*TILESIZE, 14*TILESIZE),
                (25*TILESIZE, 14*TILESIZE)]
MAX_SPEED = 9
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.topleft = (random.randint(0, 1000), (random.randint(0, 450)))
        self.rect.topleft = random.choice(enemy_coords)
        if self.rect.topleft in enemy_coords:
            enemy_coords.remove(self.rect.topleft)
        ...
        for a in collided_enemies:
            for b in collided_enemies:
                if a != b and collided_enemies:
                    for i in range(int(self.direction.length()) + 1):
    ...
    def update(self, events, dt):
        ...
        for a in player_collide:
            for b in player_collide:
                if a != b and player_collide and pressed[pygame.K_w]:
                    self.w = False
                    if self.direction in DIRECTION_LIST:
                        self.hit_direction = tuple(self.direction)
                        for _ in range(int(self.direction.length()) + 4):
                            self.pos -= self.direction.normalize()
                        if self.direction != self.hit_direction and self.direction in DIRECTION_LIST:
                            self.w = True
